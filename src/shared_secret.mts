...
export default class SharedSecret {
	...
	public static calculateSharedSecrets({sessionKey, hopPublicKeys}: {
		sessionKey: Buffer,
		hopPublicKeys: Buffer[]
	}): Buffer[] {
		let ephemeralPrivateKey = Bigi.fromBuffer(sessionKey);
		const hopSharedSecrets = [];

		for (const [i, currentPublicKey] of hopPublicKeys.entries()) {
			debug('Round %d', i);

			const hopPublicKey = ecurve.Point.decodeFrom(secp256k1, currentPublicKey);
	...
	private static calculateBlindingFactor({ephemeralPublicKey, sharedSecret}: {
	...
	public static calculateNextEphemeralPublicKey({ephemeralPublicKey, sharedSecret}: {
